          ...
          <tbody>
            {ents.map((e) => (
              <tr key={e.id} className="border-b border-neutral-200 align-top">
                <td className="py-2 pr-3"><Link className="underline" href={`/entidade/${e.id}`}>{e.nome}</Link></td>
                <td className="py-2 pr-3">{e.tipo}{e.origem === "documento" ? " (terceiro mencionado)" : ""}</td>
                ...
                <td className="py-2 pr-3">{e.assercoes}</td>
              </tr>
            ))}
          </tbody>
        </table>
