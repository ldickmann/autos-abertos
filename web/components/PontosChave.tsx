...
export function PontosChave({ titulo = "Pontos-chave", itens, nota, id = "pontos-chave" }: { titulo?: string; itens: Ponto[]; nota?: ReactNode; id?: string }) {
  ...
  return (
    <section aria-labelledby={id} className="folha max-w-3xl border border-neutral-300 bg-white p-4 sm:p-5" style={{ borderLeft: "6px solid var(--marca)" }}>
      <h2 id={id} className="text-lg">{titulo}</h2>
      <ul className="mt-2 space-y-2.5">
        {itens.map((p, i) => (
          <li key={i} className="leitura text-base">
            {p.texto}
            {p.provas && p.provas.length > 0 && (
              <span className="ml-1 inline-flex flex-wrap gap-1 align-middle text-[11px] leading-none">
                {p.provas.map((pr, k) => (
