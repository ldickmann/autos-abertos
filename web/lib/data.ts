...
import type { Assercao, Aviso, CronologiaDados, Decisoes, DocumentoCompleto, Entidade, FonteExterna, FluxosDados, Materia, Grafo, Prova, TrajetosDados, Integridade, LinhaTempo, Meta, Processo, ProcessoResumo, Referencias, RodadaMudancas, Verbete } from "@/lib/tipos";
...
export const getTrajetos = () => ler<TrajetosDados>("trajetos.json");
export const getPontosChave = (): Record<string, { texto: string; provas: Prova[] }[]> => (fs.existsSync(path.join(DATA_DIR, "pontos_chave.json")) ? ler("pontos_chave.json") : {});
...
export const getCronologia = () => ler<CronologiaDados>("cronologia.json");
export const getFontesExternas = () => ler<FonteExterna[]>("fontes_externas.json");
export const getLegislativo = () => ler<Materia[]>("legislativo.json");
