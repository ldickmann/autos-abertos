...
def exportar(con: sqlite3.Connection, saida: Path, *, semente: int) -> dict:
    ...
    from .fluxos_export import atores_csv, bens_csv, comunicacoes_csv, exportar_fluxos, fluxos_csv
    ...
    if trajetos_path.exists() and con.execute("SELECT COUNT(*) FROM fluxo_fonte").fetchone()[0]:
