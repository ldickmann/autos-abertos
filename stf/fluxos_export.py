...
def exportar_fluxos(con: sqlite3.Connection) -> dict:
    ...
    transacoes = _rows(con, """
        SELECT id, comunicacao_id, origem_ator_id, destino_ator_id, valor_centavos, data, periodo_inicio, periodo_fim, tipo, natureza, quantidade,
               bem_id, descricao, pagina, trecho_fonte, situacao
    ...
    for t in transacoes:
        ...
        if t["natureza"] != "resumo_tipo" and t["situacao"] == "efetuado":   # resumos por tipo repetem os agregados; previstos/cobrados não são dinheiro pago
    ...
    for t in transacoes:
        if t["natureza"] == "resumo_tipo" or t["situacao"] != "efetuado" or t["origem_ator_id"] is None or t["destino_ator_id"] is None:
            continue
    ...
    resumo = {
        ...
        "por_secao": {s: sum(1 for c in comunicacoes if c["secao"] == s) for s in ("suspeita", "automatica", "especie", "relatorio")},
    }
...
def fluxos_csv(con: sqlite3.Connection) -> str:
    ...
    w.writerow(["fonte", "comunicacao", "secao", "origem", "destino", "valor_reais", "data", "periodo_inicio", "periodo_fim", "tipo", "natureza",
                "quantidade", "pagina", "trecho", "descricao", "origem_documento", "destino_documento", "documento_id", "situacao"])
    for r in con.execute("""
        ...
        w.writerow([r["identificador"], r["numero"], r["secao"], r["origem"], r["destino"], f"{r['valor_centavos'] / 100:.2f}", r["data"],
                    ...
                    r["descricao"], r["od"], r["dd"], r["documento_id"], r["situacao"]])
    return out.getvalue()
...
def atores_csv(con: sqlite3.Connection) -> str:
    ...
    for r in con.execute("""
        SELECT a.id, a.nome, a.tipo, a.documento_mascarado, a.atividade, a.entidade_id,
               ...
               COALESCE((SELECT SUM(valor_centavos) FROM fluxo_transacao t WHERE t.destino_ator_id=a.id AND t.natureza!='resumo_tipo' AND t.situacao='efetuado'), 0) AS recebeu,
               COALESCE((SELECT SUM(valor_centavos) FROM fluxo_transacao t WHERE t.origem_ator_id=a.id AND t.natureza!='resumo_tipo' AND t.situacao='efetuado'), 0) AS pagou,
